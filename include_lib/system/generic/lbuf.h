 ...
 * @brief 链表buf初始化
 *
 * @param buf 需要lbuf进行管理的内存
 * @param len 内存长度
 * @param align 输入对管理的内存进行对齐的参数,避免后续使用因地址不对齐产生碎片
 * @param priv_head_len 要管理的一个数据包结构体的最小的长度
 *
 * @return lbuf操作句柄
 */
/* --------------------------------------------------------------------------*/
 ...
 *
 * @param lbuf lbuf_alloc()返回用于存储数据包的地址
 * @param size 需重新分配的空间的字节长度.注:size的大小只能比lbuf_alloc()中的len小,即只能重新分配更小的lbuf空间,不能扩大空间.
 *
 ...
 *
 * @param lbuf lbuf_alloc()返回用于存储数据包的地址
 * @param channel_map 选择映射到哪个通道,最多8个通道,使用位映射的方式进行通道对应.
 */
/* --------------------------------------------------------------------------*/
 ...
 *
 * @param head lbuf操作句柄
 * @param channel 需要读取的通道值,一般使用BIT(n),n为需要读取的通道
 *
 ...
 *
 * @param lbuf lbuf_alloc()返回用于存储数据包的地址
 * @param rets 调用lbuf_free_check()函数的返回地址rets,取值可参考lbuf_free()
 */
/* --------------------------------------------------------------------------*/
