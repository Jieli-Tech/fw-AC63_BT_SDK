...
/*单工通话接口*/
s32 aec_spx_init(struct aec_s_attr *attr);
s32 aec_spx_exit();
u32 aec_spx_output_read(s16 *buf, u16 npoint);
s32 aec_spx_fill_in_data(void *dat, u16 len);
s32 aec_spx_fill_ref_data(void *dat, u16 len);
void aec_spx_toggle(u8 toggle);

int sms_init(struct aec_s_attr *attr);
int sms_exit();
int sms_fill_in_data(u8 *dat, u16 len);
int sms_fill_ref_data(u8 *dat, u16 len);
void sms_toggle(u8 toggle);
