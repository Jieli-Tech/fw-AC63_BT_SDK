...
CHARGE_PLATFORM_DATA_BEGIN(charge_data)
	...
	const struct iokey_port iokey_list[] = {
		...
		{
			.connect_way = ONE_PORT_TO_LOW,                     //IO按键的连接方式
			.key_type.one_io.port = TCFG_IOKEY_MOUSE_FB_PORT,   //IO按键对应的引脚
			.key_value = KEY_FB_VAL,                            //按键值
		},
		{
			.connect_way = ONE_PORT_TO_LOW,                     //IO按键的连接方式
			.key_type.one_io.port = TCFG_IOKEY_MOUSE_BB_PORT,   //IO按键对应的引脚
			.key_value = KEY_BB_VAL,                            //按键值
		},
...
struct port_wakeup port5 = {
	...
	.both_edge          = 0,
	.filter             = PORT_FLT_2ms,
	.iomap              = TCFG_IOKEY_MOUSE_FB_PORT,                       //唤醒口选择
};
...
struct port_wakeup port6 = {
	...
	.both_edge          = 0,
	.filter             = PORT_FLT_2ms,
	.iomap              = TCFG_IOKEY_MOUSE_BB_PORT,                       //唤醒口选择
};
...
void gpio_init(void)
{
	gpio_set_dieh(TCFG_IOKEY_MOUSE_24GLED_PORT, 0);
	gpio_set_die(TCFG_IOKEY_MOUSE_24GLED_PORT, 1);
	gpio_set_pull_down(TCFG_IOKEY_MOUSE_24GLED_PORT, 1);
	gpio_set_pull_up(TCFG_IOKEY_MOUSE_24GLED_PORT, 0);
	gpio_set_direction(TCFG_IOKEY_MOUSE_24GLED_PORT, 0);
	gpio_set_output_value(TCFG_IOKEY_MOUSE_24GLED_PORT,0);


	gpio_set_dieh(TCFG_IOKEY_MOUSE_EDRLED_PORT, 0);
	gpio_set_die(TCFG_IOKEY_MOUSE_EDRLED_PORT, 1);
	gpio_set_pull_down(TCFG_IOKEY_MOUSE_EDRLED_PORT, 1);
	gpio_set_pull_up(TCFG_IOKEY_MOUSE_EDRLED_PORT, 0);
	gpio_set_direction(TCFG_IOKEY_MOUSE_EDRLED_PORT, 0);
	gpio_set_output_value(TCFG_IOKEY_MOUSE_EDRLED_PORT,0);

	gpio_set_dieh(TCFG_IOKEY_MOUSE_CPI_PORT, 0);
	gpio_set_die(TCFG_IOKEY_MOUSE_CPI_PORT, 1);
	gpio_set_pull_down(TCFG_IOKEY_MOUSE_CPI_PORT, 1);
	gpio_set_pull_up(TCFG_IOKEY_MOUSE_CPI_PORT, 0);
	gpio_set_direction(TCFG_IOKEY_MOUSE_CPI_PORT, 1);

	gpio_set_dieh(TCFG_IOKEY_MOUSE_MODE_PORT, 0);
	gpio_set_die(TCFG_IOKEY_MOUSE_MODE_PORT, 1);
	gpio_set_pull_down(TCFG_IOKEY_MOUSE_MODE_PORT, 0);
	gpio_set_pull_up(TCFG_IOKEY_MOUSE_MODE_PORT, 1);
	gpio_set_direction(TCFG_IOKEY_MOUSE_MODE_PORT, 1);
...
static void close_gpio(u8 mode)
{
	...
	if(!mode)
	{
		port_protect(port_group, TCFG_IOKEY_MOUSE_24GLED_PORT);
		port_protect(port_group, TCFG_IOKEY_MOUSE_EDRLED_PORT);
		port_protect(port_group, TCFG_IOKEY_MOUSE_CPI_PORT);
	}
