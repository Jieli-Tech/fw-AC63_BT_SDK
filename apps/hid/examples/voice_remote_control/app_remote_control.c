...
#define LOG_TAG_CONST       HID_VRC
#define LOG_TAG             "[HID_VRC]"
...
//默认配置
static const edr_sniff_par_t hidvrc_sniff_param = {
...
static volatile u8 is_hidvrc_active = 0;//1-临界点,系统不允许进入低功耗，0-系统可以进入低功耗
static u16 g_auto_shutdown_timer = 0;
static void hidvrc_app_select_btmode(u8 mode);

//----------------------------------
static const u8 hidvrc_report_map[] = {
...
static const edr_init_cfg_t hidvrc_edr_config = {
    ...
    .report_map = hidvrc_report_map,
    .report_map_size = sizeof(hidvrc_report_map),
};

//----------------------------------
static const ble_init_cfg_t hidvrc_ble_config = {
    .same_address = 0,
 ...
 */
/*************************************************************************************************/
static void hidvrc_auto_shutdown_disable(void)
{
...
void hidvrc_test_keep_send_data(void)
{
 ...
 */
/*************************************************************************************************/
void hidvrc_test_keep_send_init(void)
{
    if (bt_hid_mode == HID_MODE_BLE) {
#if TCFG_USER_BLE_ENABLE
        log_info("###keep test ble\n");
        ble_hid_timer_handle = sys_s_hi_timer_add((void *)0, hidvrc_test_keep_send_data, 10);
#endif
    } else {
#if TCFG_USER_EDR_ENABLE
        log_info("###keep test edr\n");
        edr_hid_timer_handle = sys_s_hi_timer_add((void *)0, hidvrc_test_keep_send_data, 10);
#endif
    }
}
#endif
...
static void hidvrc_app_key_deal_test(u8 key_type, u8 key_value)
{
    ...
    if (key_type == KEY_EVENT_DOUBLE_CLICK && key_value == TCFG_ADKEY_VALUE0) {
        ...
        if (HID_MODE_BLE == bt_hid_mode) {
            hidvrc_app_select_btmode(HID_MODE_EDR);
        } else {
            hidvrc_app_select_btmode(HID_MODE_BLE);
        }
...
static void hidvrc_vm_deal(u8 rw_flag)
{
    hid_vm_cfg_t info;
    int ret;
...
static void hidvrc_set_soft_poweroff(void)
{
    log_info("hidvrc_set_soft_poweroff\n");
    is_hidvrc_active = 1;
...
static void hidvrc_timer_handle_test(void)
{
    log_info("not_bt");
}
...
#if TCFG_USER_EDR_ENABLE
    btstack_edr_start_before_init(&hidvrc_edr_config, 0);
#endif

#if TCFG_USER_BLE_ENABLE
    btstack_ble_start_before_init(&hidvrc_ble_config, 0);
...
#else
    //no bt,to for test
    sys_timer_add(NULL, hidvrc_timer_handle_test, 1000);
#endif
 ...
 */
/*************************************************************************************************/
static int hidvrc_state_machine(struct application *app, enum app_state state, struct intent *it)
{
    switch (state) {
    case APP_STA_CREATE:
        break;
 ...
 */
/*************************************************************************************************/
static int hidvrc_bt_hci_event_handler(struct bt_event *bt)
{
 ...
 */
/*************************************************************************************************/
static int hidvrc_bt_connction_status_event_handler(struct bt_event *bt)
{
    ...
    case BT_STATUS_INIT_OK:
        ...
        hidvrc_vm_deal(0);//bt_hid_mode read for VM
...
//根据模式执行对应蓝牙的初始化
        ...
        hidvrc_app_select_btmode(HID_MODE_INIT);//

#if HID_TEST_KEEP_SEND_EN
        hidvrc_test_keep_send_init();
#endif
        break;
 ...
 */
/*************************************************************************************************/
static int hidvrc_bt_common_event_handler(struct bt_event *bt)
{
    log_info("----%s reason %x %x", __FUNCTION__, bt->event, bt->value);
 ...
 */
/*************************************************************************************************/
static void hidvrc_key_event_handler(struct sys_event *event)
{
    /* u16 cpi = 0; */
    u8 event_type = 0;
    u8 key_value = 0;
    ...
    if (event->arg == (void *)DEVICE_EVENT_FROM_KEY) {
        ...
        hidvrc_app_key_deal_test(event_type, key_value);
    }
}
 ...
 */
/*************************************************************************************************/
static int hidvrc_event_handler(struct application *app, struct sys_event *event)
{
#if (TCFG_HID_AUTO_SHUTDOWN_TIME)
    //重置无操作定时计数
    ...
    switch (event->type) {
    case SYS_KEY_EVENT:
        hidvrc_key_event_handler(event);
        return 0;

    case SYS_BT_EVENT:
#if (TCFG_USER_EDR_ENABLE || TCFG_USER_BLE_ENABLE)
        if ((u32)event->arg == SYS_BT_EVENT_TYPE_CON_STATUS) {
            hidvrc_bt_connction_status_event_handler(&event->u.bt);
        } else if ((u32)event->arg == SYS_BT_EVENT_TYPE_HCI_STATUS) {
            hidvrc_bt_hci_event_handler(&event->u.bt);
        } else if ((u32)event->arg == SYS_BT_EVENT_FORM_COMMON) {
            return hidvrc_bt_common_event_handler(&event->u.dev);
        }
#endif
        return 0;
    ...
    case SYS_DEVICE_EVENT:
        if ((u32)event->arg == DEVICE_EVENT_FROM_POWER) {
            return app_power_event_handler(&event->u.dev, hidvrc_set_soft_poweroff);
        }
...
static u8 hidvrc_app_idle_query(void)
{
    return !is_hidvrc_active;
}

REGISTER_LP_TARGET(app_hidvrc_lp_target) = {
    .name = "app_hidvrc_deal",
    .is_idle = hidvrc_app_idle_query,
};


static const struct application_operation app_hidvrc_ops = {
    .state_machine  = hidvrc_state_machine,
    .event_handler 	= hidvrc_event_handler,
};

/*
 * 注册模式
 */
REGISTER_APPLICATION(app_hidvrc) = {
    .name 	= "hid_vrc",
