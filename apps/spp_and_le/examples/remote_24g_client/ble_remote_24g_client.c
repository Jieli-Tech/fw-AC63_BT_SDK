...
#if CONFIG_APP_REMOTE_24G_C && TCFG_USER_BLE_ENABLE

#if LE_DEBUG_PRINT_EN
#define log_info(x, ...)  printf("[BLE_REMOTE]" x " ", ## __VA_ARGS__)
...
//---------------------------------------------------------------------------
static scan_conn_cfg_t remote_central_scan_cfg;
static u16 remote_ble_central_write_handle;
static u16 remote_conn_handle;
...
static u16 remote_timer_id = 0;

//remote 上电开配对管理,若配对失败,没有配对设备，停止搜索
...
static const sm_cfg_t remote_sm_init_config = {
...
static gatt_ctrl_t remote_gatt_control_block = {
    //public
    ...
    .sm_config = &remote_sm_init_config,
#else
    .sm_config = NULL,
#endif
    //cbk,event handle
...
static int remote_central_event_packet_handler(int event, u8 *packet, u16 size, u8 *ext_param);

static const gatt_client_cfg_t central_client_init_cfg = {
    .event_packet_handler = remote_central_event_packet_handler,
};
...
//---------------------------------------------------------------------------
//指定搜索uuid
static const target_uuid_t  remote_24g_search_ble_uuid_table[] = {
    /* { */
...
//配置多个扫描匹配设备
static const u8 remote_test_remoter_name1[] = "REMOTE_24G(BLE)";//
static const u8 remote_test_remoter_name2[] = "AC630N_mx(BLE)";//
static const u8 user_config_tag_string[] = "abc123";
static const client_match_cfg_t remote_match_device_table[] = {
    {
        .create_conn_mode = BIT(CLI_CREAT_BY_NAME),
        .compare_data_len = sizeof(remote_test_remoter_name1) - 1, //去结束符
        .compare_data = remote_test_remoter_name1,
    },
    ...
    {
        .create_conn_mode = BIT(CLI_CREAT_BY_NAME),
        .compare_data_len = sizeof(remote_test_remoter_name2) - 1, //去结束符
        .compare_data = remote_test_remoter_name2,
    },
};


static const gatt_search_cfg_t remote_central_search_config = {
    .match_devices = &remote_match_device_table,
    .match_devices_count = (sizeof(remote_match_device_table) / sizeof(client_match_cfg_t)),
    .match_rssi_enable = 1,
    .match_rssi_value = MATCH_DEVICE_RSSI_LEVEL,

#if SEARCH_PROFILE_EN
    .search_uuid_count = (sizeof(remote_24g_search_ble_uuid_table) / sizeof(target_uuid_t)),
    .search_uuid_group = remote_24g_search_ble_uuid_table,
    .auto_enable_ccc = 1,
#else
    .search_uuid_count = 0,
...
//input priv
static void remote_timer_handle(u32 priv)
{
    putchar('%');
...
static const u16 remote_ccc_value = 0x0001;
static const u16 mouse_notify_handle[3] = {0x0027, 0x002b, 0x002f};
static void remote_enable_notify_ccc(void)
{
    log_info("%s\n", __FUNCTION__);
    ble_comm_att_send_data(remote_conn_handle, mouse_notify_handle[0] + 1, &remote_ccc_value, 2, ATT_OP_WRITE);
    ble_comm_att_send_data(remote_conn_handle, mouse_notify_handle[1] + 1, &remote_ccc_value, 2, ATT_OP_WRITE);
    ble_comm_att_send_data(remote_conn_handle, mouse_notify_handle[2] + 1, &remote_ccc_value, 2, ATT_OP_WRITE);
}

static int remote_pair_vm_do(u8 *info, u8 info_len, u8 rw_flag)
{
    int ret;
    int vm_len = info_len;
...
{
    if (remote_conn_handle && remote_ble_central_write_handle) {
        return ble_comm_att_send_data(remote_conn_handle, remote_ble_central_write_handle, data, len, ATT_OP_WRITE);
    }
...
extern void remote_24g_ble_hid_input_handler(u8 *packet, u16 size);
static int remote_central_event_packet_handler(int event, u8 *packet, u16 size, u8 *ext_param)
{
    ...
    case GATT_COMM_EVENT_GATT_DATA_REPORT: {
        ...
        case GATT_EVENT_NOTIFICATION: { //notify
            ...
            remote_24g_ble_hid_input_handler(packet, report_data->blob_length + 1);
            putchar('&');
        }
        break;
    ...
    case GATT_COMM_EVENT_CONNECTION_COMPLETE:
        ...
        remote_conn_handle = little_endian_read_16(packet, 0);
    ...
    case GATT_COMM_EVENT_ENCRYPTION_CHANGE:
        ...
        if (packet[3] == LINK_ENCRYPTION_RECONNECT) {
            remote_enable_notify_ccc();
        } else {
            memcpy(&pair_bond_info[1], cur_peer_addr_info, 7);
            pair_bond_info[0] = PAIR_BOND_TAG;
            remote_pair_vm_do(pair_bond_info, sizeof(pair_bond_info), 1);
...
//广播参数设置
static void remote_scan_conn_config_set(void)
{
    remote_central_scan_cfg.scan_auto_do = 1;
    remote_central_scan_cfg.creat_auto_do = 1;
    remote_central_scan_cfg.scan_type = SET_SCAN_TYPE;
    remote_central_scan_cfg.scan_filter = 1;
    remote_central_scan_cfg.scan_interval = SET_SCAN_INTERVAL;
    remote_central_scan_cfg.scan_window = SET_SCAN_WINDOW;

    remote_central_scan_cfg.creat_conn_interval = SET_CONN_INTERVAL;
    remote_central_scan_cfg.creat_conn_latency = SET_CONN_LATENCY;
    remote_central_scan_cfg.creat_conn_super_timeout = SET_CONN_TIMEOUT;
    remote_central_scan_cfg.conn_update_accept = 1;
    remote_central_scan_cfg.creat_state_timeout_ms = SET_CREAT_CONN_TIMEOUT;

    ble_gatt_client_set_scan_config(&remote_central_scan_cfg);
}

static void remote_central_init(void)
{
    log_info("%s", __FUNCTION__);
    remote_pair_vm_do(pair_bond_info, sizeof(pair_bond_info), 0);
    ble_gatt_client_set_search_config(&remote_central_search_config);
    remote_scan_conn_config_set();
}
...
{
    ble_comm_init(&remote_gatt_control_block);
}
...
void bt_ble_init(void)
{
    ...
    if (pair_bond_info[0] == PAIR_BOND_TAG) {
        ...
        remote_timer_handle(0);
#endif
        remote_timer_id = sys_timer_add((void *)1, remote_timer_handle, POWER_ON_SWITCH_TIME);
    } else {
        ble_gatt_client_scan_enable(1);
        log_info("just scan");
    }
