...
#if CONFIG_APP_REMOTE_24G_C && TCFG_PC_ENABLE && TCFG_USB_SLAVE_USER_HID
...
extern void remote_24g_hid_output_handler(u8 report_type, u8 report_id, u8 *packet, u16 size);
static u32 hid_recv_output_report(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    remote_24g_hid_output_handler(0x52, read_ep[0], &read_ep[1], setup->wLength - 1);
    return ret;
}
