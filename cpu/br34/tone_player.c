...
#else
        wlen = audio_output_data(data, remain_len);
#endif
    }
...
static int tone_file_dec_src_output_handler(void *priv, void *data, int len)
{
    int wlen = 0;
    int rlen = len;
...
#if AUDIO_OUT_MIXER_ENABLE
    return audio_mixer_ch_write(&file_dec->mix_ch, data, rlen);
#else
    return audio_output_data(data, rlen);
#endif
}
...
#else
    if (!file_dec->audio_sync && (!file_dec->dec_mix || !bt_audio_is_running())) {
        file_dec->audio_sync = local_file_dec_sync_open(file_dec->ch_num, fmt->sample_rate, fmt->sample_rate);
    }
    if (file_dec->audio_sync) {
        audio_dac_sync_start(&dac_hdl);
    }
