...
#define RECEPTION_NRF_NUMBER         32//how much data to receive?(unit: bit)
#define BUF_NFC_NUM                  (RECEPTION_NRF_NUMBER / 8) + ((RECEPTION_NRF_NUMBER % 8 == 0) ? 0 : 1)
...
*/
___interrupt
void timer_cap_nrf_isr(void)
{
    u32 cur_cnt = 0;
    if ((JL_TIMER2->CON & BIT(1)) && (JL_TIMER2->CON & BIT(15))) {
        JL_TIMER2->CON |= BIT(14);
        cur_cnt = JL_TIMER2->PRD;
        JL_TIMER2->CNT = 0;
        if (tmrcap_isr_cbfun[2]) {
            u8 cur_edge = !!(JL_TIMER2->CON & BIT(0));
            tmrcap_isr_cbfun[2](cur_edge, cur_cnt);
        }
        if (tmrcap_edge[2] > 1) {
            JL_TIMER2->CON ^= BIT(0);
        }
 ...
 */
void timer_cap_nrf_init(JL_TIMER_TypeDef *JL_TIMERx, u32 cap_io, u8 edge, CLK_DIV_4bit clk_div, void (*cbfun)(u8, u32))
{
    u8 tmr_num;
    ...
    request_irq(TIMERx_IRQ_IDX[tmr_num], 3, timer_cap_nrf_isr, 0);
 ...
 * @param JL_TIMERx : JL_TIMER0/1/2/3/
 */
void close_timer_nrf_cap(JL_TIMER_TypeDef *JL_TIMERx)
{
    JL_TIMERx->CON &= ~BIT(0);
}

/************************************ 以下SDK的参考示例 ****************************************/
static u8 nfc_buffer[BUF_NFC_NUM];
static void user_tmr3_cap_cbfun(u8 edge, u32 cnt)
{
    /* printf("t2 %d %d\n", edge, cnt); */
#if TCFG_NFC_IO_TEST
    if (!edge) {
        gpio_write(IO_PORTA_06, 1);
    } else {
        gpio_write(IO_PORTA_06, 0);
    }
#endif
    //notify app
    /* return_nfc_notify(nfc_buffer, BUF_NFC_NUM); */
}

void timer_cap_nrf_test(void)
{
    printf("*********** timer cap nrf test *************\n");

#if TCFG_NFC_IO_TEST
    gpio_set_direction(IO_PORTA_06, 0);
#endif

    timer_cap_nrf_init(JL_TIMER2, TCFG_NFC_PORT, 3, CLK_DIV_4, user_tmr3_cap_cbfun);
