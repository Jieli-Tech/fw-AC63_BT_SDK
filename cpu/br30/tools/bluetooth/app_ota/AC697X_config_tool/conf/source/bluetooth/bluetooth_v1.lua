...
for i = 1, 20 do
	local c = cfg:str("蓝牙名字" .. i, "AC697X_" .. i);
	local on = 0;
...
local ul_eq_en = {};
ul_eq_en.cfg = cfg:enum("UL_EQ_EN: ", UL_EQ_EN_SEL_TABLE, 0);
depend_item_en_show(bluetooth_en, ul_eq_en.cfg);
ul_eq_en.cfg:setOSize(1);
-- item_htext
ul_eq_en.comment_str = item_comment_context("UL_EQ_EN", ul_eq_en_sel_table) .. "默认值：0";
...
ul_eq_en.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("(上行 EQ 使能，默认值：disable)"),
    cfg:stSpacer(),
};
...
-- 5-1) ndt_fade_in
fade_gain.ndt_fade_in.cfg = cfg:dbf("NDT_FADE_IN：", 1.0);
depend_item_en_show(bluetooth_en, fade_gain.ndt_fade_in.cfg);
fade_gain.ndt_fade_in.cfg:setOSize(4);
-- item_htext
fade_gain.ndt_fade_in.comment_str = "设置范围：0.1 ~ 5 dB, float类型(4Bytes), 默认值：1.0";
...
fade_gain.ndt_fade_in.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("dB (单端讲话淡入步进，设置范围: 0.1 ~ 5 dB，默认值：1.0 dB)"),
    cfg:stSpacer(),
};

-- 5-2) ndt_fade_out
fade_gain.ndt_fade_out.cfg = cfg:dbf("NDT_FADE_OUT：", 0.5);
depend_item_en_show(bluetooth_en, fade_gain.ndt_fade_out.cfg);
fade_gain.ndt_fade_out.cfg:setOSize(4);
-- item_htext
fade_gain.ndt_fade_out.comment_str = "设置范围：0.1 ~ 5 dB, float类型(4Bytes), 默认值：0.5";
...
fade_gain.ndt_fade_out.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("dB (单端讲话淡出步进，设置范围: 0.1 ~ 5 dB，默认值：0.5 dB)"),
    cfg:stSpacer(),
};

-- 5-3) dt_fade_in
fade_gain.dt_fade_in.cfg = cfg:dbf("DT_FADE_IN：", 1.0);
depend_item_en_show(bluetooth_en, fade_gain.dt_fade_in.cfg);
fade_gain.dt_fade_in.cfg:setOSize(4);
-- item_htext
fade_gain.dt_fade_in.comment_str = "设置范围：0.1 ~ 5 dB, float类型(4Bytes), 默认值：1.0";
...
fade_gain.dt_fade_in.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("dB (双端讲话淡入步进，设置范围: 0.1 ~ 5 dB，默认值：1.0 dB)"),
    cfg:stSpacer(),
};

-- 5-4) dt_fade_out
fade_gain.dt_fade_out.cfg = cfg:dbf("DT_FADE_OUT：", 0.5);
depend_item_en_show(bluetooth_en, fade_gain.dt_fade_out.cfg);
fade_gain.dt_fade_out.cfg:setOSize(4);
-- item_htext
fade_gain.dt_fade_out.comment_str = "设置范围：0.1 ~ 5 dB, float类型(4Bytes), 默认值：0.5";
...
fade_gain.dt_fade_out.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("dB (双端讲话淡出步进，设置范围: 0.1 ~ 5 dB，默认值：0.5 dB)"),
    cfg:stSpacer(),
};
...
ndt_speech_thr.cfg = cfg:dbf("NDT_SPEECH_THR：", -60.0);
depend_item_en_show(bluetooth_en, ndt_speech_thr.cfg);
ndt_speech_thr.cfg:setOSize(4);
-- item_htext
ndt_speech_thr.comment_str = "设置范围：-70.0 ~ -40.0 dB, 默认值：-60.0 dB";
...
ndt_speech_thr.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("(单端讲话放大阈值， 设置范围: -70.0 ~ -40.0 dB，默认值：-60.0 dB)"),
    cfg:stSpacer(),
};
...
-- 11) dt_speech_thr
local dt_speech_thr = {};
dt_speech_thr.cfg = cfg:dbf("DT_SPEECH_THR：", -50.0);
depend_item_en_show(bluetooth_en, dt_speech_thr.cfg);
dt_speech_thr.cfg:setOSize(4);
-- item_htext
dt_speech_thr.comment_str = "设置范围：-70.0 ~ -40.0 dB, 默认值：-50.0 dB";
...
dt_speech_thr.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("(双端讲话放大阈值， 设置范围: -70.0 ~ -40.0 dB，默认值：-50.0 dB)"),
    cfg:stSpacer(),
};

-- 12) echo_present_thr
local echo_present_thr = {};
echo_present_thr.cfg = cfg:dbf("ECHO_PRESENT_THR：", -40.0);
depend_item_en_show(bluetooth_en, echo_present_thr.cfg);
echo_present_thr.cfg:setOSize(4);
-- item_htext
echo_present_thr.comment_str = "设置范围：-70.0 ~ -40.0 dB, 默认值：-40.0 dB";
...
echo_present_thr.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("(单端双端讲话阈值， 设置范围: -70.0 ~ -40.0 dB，默认值：-40.0 dB)"),
    cfg:stSpacer(),
};
...
es_min_suppress.cfg = cfg:dbf("ES_MIN_SUPPRESS：", 3.0);
depend_item_en_show(bluetooth_en, es_min_suppress.cfg);
es_min_suppress.cfg:setOSize(4);
-- item_htext
es_min_suppress.comment_str = "设置范围：0 ~ 10.0, float类型(4Bytes), 默认值：3.0";
...
es_min_suppress.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("(回音后级静态压制,越大越强， 设置范围: 0 ~ 10.0，默认值：3.0)"),
    cfg:stSpacer(),
};
...
-- 1) lrc_ws_inc
local lrc_ws_inc = cfg:i32("lrc_ws_inc", 200);
depend_item_en_show(bluetooth_en, lrc_ws_inc);
lrc_ws_inc:setOSize(2);
-- item_htext
local lrc_ws_inc_comment_str = "设置范围：200 ~ 400, 默认值：200";
...
local lrc_ws_inc_hbox_view = cfg:hBox {
    ...
    cfg:ispinView(lrc_ws_inc, 200, 400, 1),
    cfg:stLabel("（lrc窗口步进值，单位：us，设置范围: 200 ~ 400, 默认值200）");
    cfg:stSpacer(),
};

-- 2) lrc_ws_init
local lrc_ws_init = cfg:i32("lrc_ws_init", 200);
depend_item_en_show(bluetooth_en, lrc_ws_init);
lrc_ws_init:setOSize(2);
-- item_htext
local lrc_ws_init_comment_str = "设置范围：200 ~ 400, 默认值：400";
...
local lrc_ws_init_hbox_view = cfg:hBox {
    ...
    cfg:ispinView(lrc_ws_init, 200, 400, 1),
    cfg:stLabel("（lrc窗口初始值，单位：us，设置范围: 200 ~ 400, 默认值200）");
    cfg:stSpacer(),
};

-- 3) btosc_ws_inc
local btosc_ws_inc = cfg:i32("btosc_ws_inc", 200);
depend_item_en_show(bluetooth_en, btosc_ws_inc);
btosc_ws_inc:setOSize(2);
-- item_htext
local btosc_ws_inc_comment_str = "设置范围：200 ~ 400, 默认值：200";
...
local btosc_ws_inc_hbox_view = cfg:hBox {
    ...
    cfg:ispinView(btosc_ws_inc, 200, 400, 1),
    cfg:stLabel("（btosc窗口步进值，单位：us，设置范围: 200 ~ 400, 默认值200）");
    cfg:stSpacer(),
};

-- 4) btosc_ws_init
local btosc_ws_init = cfg:i32("btosc_ws_init", 200);
depend_item_en_show(bluetooth_en, btosc_ws_init);
btosc_ws_init:setOSize(2);
-- item_htext
local btosc_ws_init_comment_str = "设置范围：200 ~ 400, 默认值：200";
...
local btosc_ws_init_hbox_view = cfg:hBox {
    ...
    cfg:ispinView(btosc_ws_init, 200, 400, 1),
    cfg:stLabel("（btosc窗口初始值，单位：us，设置范围: 200 ~ 400, 默认值200）");
    cfg:stSpacer(),
};
